...
- 本项目仅用于学习与研究目的，风险自负。
## 风险敞口说明
- 当前风险敞口：指账户当前承受的潜在风险比例，计算公式为 `risk_percentage = total_risk / account_balance`。其中 `total_risk` 是所有持仓的风险金额之和，风险金额定义为 `risk_amount = position_size × |entry_price − stop_loss|`。该指标反映若所有持仓同时到达止损位时的潜在亏损占账户余额的比例。
- 风险敞口添加：开仓时会将该笔交易的风险金额加入到 `total_risk`（即“增加敞口”）；平仓时会将该持仓对应的风险金额从 `total_risk` 中扣除（即“释放敞口”）。例如，账户余额 10,000，买入 1 手、止损距离 50，则本笔风险金额为 50，当前风险敞口增加为 `50/10000=0.005`（0.5%）。多笔持仓的风险敞口累加后不得超过配置的总风险上限。

## 日亏损限额（按总额百分比）
- 可通过 `daily_loss_limit_pct` 设置日亏损限额占比，例如 `0.05` 表示当日净亏损达到账户余额的 5% 即触发限额。
- 风险管理器会在每次平仓后累计当日净值变动，若当日净亏损的绝对值超过 `account_balance × daily_loss_limit_pct`，则视为达到日亏损限额（可用于暂停交易或禁止新开仓）。